...
<div class="crack-form">
	...
	?>
		...
		echo $form->field($model, 'gen_id')->widget(Select2::classname(), [
			'data' => ArrayHelper::map(Generator::find()->orderBy(['name' => SORT_ASC])->all(), 'id', 'name'),
		...
		echo $form->field($model, 'algo_id')->widget(Select2::classname(), [
			'data' => ArrayHelper::map(Algorithm::find()->orderBy(['name' => SORT_ASC])->all(), 'id', 'name'),
			'options' => ['placeholder' => 'Select...']
		]);
		?>
		...
		<fieldset>
			...
			<legend style="font-size: medium;">
					...
					echo $form->field($model, 'mode', [
						...
						'pluginOptions' => [
							'offText' => 'Simple',
							'onText' => '&nbsp;Mask&nbsp;' // Add space to match it's length to offText to be displayed correctly!
						]
					]);
					?>
			</legend>
